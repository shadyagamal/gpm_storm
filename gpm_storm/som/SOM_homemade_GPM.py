...
def preprocess_data(df, vars):
    ...
    df_uniform = inverse_density_sample_df(df_thresh)
...
def update_learning_rates(epoch, n_epochs, initial_eta, initial_sigma):
    eta = initial_eta * (1 - epoch / n_epochs)
    sigma = initial_sigma * (1 - epoch / n_epochs)
    return eta, sigma
...
df_scaled, df_original, df_uniform = preprocess_data(df, vars)
...
initialcodebook = np.ascontiguousarray(
    df_uniform.to_numpy().reshape(n_rows, n_columns, X.shape[1]), dtype=np.float32)

initialcodebook_random = np.random.rand(n_rows, n_columns, X.shape[1])
initialcodebook_random = initialcodebook_random.astype(np.float32)

n_epochs = 50 
sigma = 10
eta = 1.1
initial_batch_size = 2880
min_batch_size=1000

W, metrics= train_som_with_convergence(X, initialcodebook, distance_matrix, 
                                             n_epochs=n_epochs, sigma=sigma, eta=eta,
