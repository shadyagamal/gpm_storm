...
def calculate_image_statistics(ds_patch, da_label):
    ...
    for threshold in thresholds:
        ds_patch[f"ED{threshold}_SOLID"] = ds_patch.gpm.retrieve(
            "EchoDepth",
            threshold=threshold,
            mask_liquid_phase=True,
        )
        ds_patch[f"ED{threshold}_FULL"] = ds_patch.gpm.retrieve(
            "EchoDepth",
            threshold=threshold,
    ...
    stats.update(
        {
            ...
            for stat, val in zip(
                ["mean", "std", "max"],
                _calculate_mean_std_max_stats(ds_patch[var].data),
                strict=False,
            )
        },
    )
    ...
    stats["WR_frac"] = (
        np.nan
        if ds_patch["heightZeroDeg"] is None
        else np.nanmean(
    ...
    stats["V_frac"] = (
        np.nan
        if ds_patch["flagPrecip"] is None
        else np.nanmean(
