...
from scipy.stats import anderson, binned_statistic_2d, shapiro
...
def _relative_distribution_of_dataset(df):
    ...
    for i, column in enumerate(columns):
        ...
        if pd.api.types.is_numeric_dtype(df[column]) and column != (
            "time",
            "along_track_start",
            "along_track_stop",
            "gpm_granule_id",
...
df_stats_pl = grouped_df.agg(
    pl.col("precipitation_average").count().alias("bin_count"),
    pl.col("row-col").mode().alias("more_frequent_node"),
)

ds = pl_df_to_xarray(df_stats_pl, xbin_column=xbin_column, ybin_column=ybin_column, bin_spacing=bin_spacing)
